package co.uk.fostorial.quest.data.repository

import co.uk.fostorial.quest.data.db.HeroClassProgressDao
import co.uk.fostorial.quest.data.db.PlayerProfileDao
...
class PlayerRepository(
    ...
    suspend fun getAllClassProgressOnce(): List<HeroClassProgress> =
        classProgressDao.getAllProgress().first()

    suspend fun restoreProfile(profile: PlayerProfile, classProgressList: List<HeroClassProgress>) {
        val existing = getOrCreateProfile()
        dao.updateProfile(profile.copy(id = existing.id))
        classProgressDao.clearAll()
        classProgressList.forEach { classProgressDao.upsertProgress(it) }
    }
